    ...
    try {
      ...
      if (parsed.team_id) {
        ...
        Promise.all([
          ...
          .then(async ([teamsRes, skiersRes, racesRes]) => {
            if (!teamsRes.ok || !skiersRes.ok || !racesRes.ok) {
              throw new Error("Unable to load data");
            }
            const [teamsData, skiersData, racesData] = await Promise.all([
              teamsRes.json(),
              skiersRes.json(),
              racesRes.json(),
            ]);
            ...
            setRaces(
              racesData.filter(
                (race) =>
                  Number(race.team1_id) === skierTeamId ||
                  Number(race.team2_id) === skierTeamId,
              ),
            );
            setError("");
          })
